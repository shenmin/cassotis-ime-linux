...
共享的 Free Pascal 引擎以言泉输入法 v1.26.1 为行为基线，移植了候选召回、
...
- 移植言泉输入法 v1.26.1 的短词、上下文候选和长句本地统计排序模型链；
...
v0.8.0 源码以言泉输入法和 Cassotis Lexicon v1.26.1 为行为和数据基线，
重新构建 schema 24 词库，包含 213,359 条简体词条及 216,574 条繁体词条。
相比 v0.7.0，新版加入联合长句修复与双向上下文校验，确保一键补全保留修正后的
文本，并在后台提前准备补全结果，不让每次按键等待补全推理。
持续输入中的 `zh`/`ch`/`sh` 处理和完整词片段排序得到改进，
同时修复紫光双拼的音节边界问题。简繁显示使用 OpenCC 转换，切换模式后仍保留
