...
behavior of the Cassotis IME v1.22.0 baseline. This includes the corpus-trained
...
- Local statistical ranking for short words, context-aware candidates, and
  long sentences, ported from the Cassotis IME v1.22.0 model chain. Ambiguous
...
The v0.6.0 source follows Cassotis IME v1.22.0 and Cassotis Lexicon v1.21.0,
with the same schema-24 dictionaries used by that Windows release. Compared
with v0.5.0, it adds constrained local sentence repair, background-cached
document context, and improved long-completion arbitration. A static completion
remains available while a model result is pending, and identical results do
not cause unnecessary refreshes. Low-confidence or unavailable model results
leave the existing candidates intact.
...
Native measurements on x86_64 and aarch64 use the benchmarks in
[BENCHMARK.md](BENCHMARK.md). Across 16,300 long-sentence cases, Top1/Top2
counts are 11,671/12,810 and 11,669/12,810 respectively, compared with
11,672/12,809 for Windows v1.22.0. This is not exact long-sentence parity.
The complete short-word result remains identical to Windows on both
architectures. Release checks also cover native core and dictionary tests,
package payloads, and the automated IBus/Fcitx desktop matrix. See
[COMPATIBILITY.md](COMPATIBILITY.md) for desktop and framework test coverage.

The v0.6.0 release targets amd64 and arm64 with `.deb` packages and portable
