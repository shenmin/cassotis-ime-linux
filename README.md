...
behavior of the Cassotis IME v1.18.0 baseline. This includes the corpus-trained
multi-stage ranking chain, a compact Transformer reranker for ambiguous long
sentences, and constrained asynchronous local completion. Dictionary lookup,
model inference, completion, and learning all run locally without a cloud
service, network connection, or GPU.
...
- Local statistical ranking for short words, context-aware candidates, and
  long sentences, ported from the Cassotis IME v1.18.0 model chain. Ambiguous
  long-sentence comparisons can use the same compact host-side Transformer as
  Windows; short exact-query ranking remains on its separate deterministic
  path.
...
- Controlled one-key completion and configurable keyboard shortcuts. Static
  ...
  propose one exact-lexicon continuation when the static tier has no result,
